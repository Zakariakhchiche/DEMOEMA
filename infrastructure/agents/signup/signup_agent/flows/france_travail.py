"""Flow signup France Travail — francetravail.io (API offres d'emploi, ROME, etc.).

Signup gratuit, création d'une app donne CLIENT_ID + CLIENT_SECRET pour :
- API Offres d'emploi v2
- ROME 4.0 (métiers + compétences)
- La Bonne Boîte
- Marché du travail

⚠️ Validation email requise, activation peut prendre quelques heures.
"""
...
log = logging.getLogger("flows.france_travail")

PORTAL_URL = "https://francetravail.io/"
SIGNUP_URL = "https://francetravail.io/inscription"
LOGIN_URL = "https://francetravail.io/connexion"
...
async def run(profile, audit_dir: Path, write_credential) -> dict:
    ...
    async with async_playwright() as p:
        ...
        try:
            with MailBox(profile.project_email_imap_host, port=profile.project_email_imap_port).login(
                ...
                for msg in mailbox.fetch(
                    AND(from_="francetravail", date_gte=start_time.date()),
                    reverse=True,
                    limit=5,
                ):
                    urls = re.findall(r"https?://[^\s\"'<>]+", msg.text or msg.html or "")
                    for u in urls:
                        if "francetravail" in u and ("verify" in u or "confirm" in u or "activate" in u):
                            verif_link = u
                            break
                    if verif_link:
                        break
        ...
        return {
            ...
            "audit_dir": str(audit_dir),
            "next_steps": [
                "1. Login sur https://francetravail.io (audit/password.txt)",
                "2. Créer une app 'DEMOEMA-ingestion'",
                "3. Souscrire : API Offres d'emploi v2, ROME 4.0, La Bonne Boîte",
                "4. Copier CLIENT_ID + CLIENT_SECRET",
                "5. Ajouter au VPS : FRANCE_TRAVAIL_CLIENT_ID=... / FRANCE_TRAVAIL_CLIENT_SECRET=...",
            ],
        }
