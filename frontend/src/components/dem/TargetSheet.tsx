...
export function TargetSheet({ target, onClose, onPitch }: Props) {
  ...
  const [drillDirig, setDrillDirig] = useState<{
    nom: string; prenom: string; dn: string | null; data: Record<string, unknown> | null; loading: boolean;
  } | null>(null);

  const loadDrillDirig = async (nom: string, prenom: string, dn: string | null) => {
    setDrillDirig({ nom, prenom, dn, data: null, loading: true });
    try {
      const url = dn
        ? `/api/datalake/dirigeant/${encodeURIComponent(nom)}/${encodeURIComponent(prenom)}/${encodeURIComponent(dn)}`
        : `/api/datalake/dirigeant/${encodeURIComponent(nom)}/${encodeURIComponent(prenom)}`;
      const r = await fetch(url);
      if (!r.ok) {
        setDrillDirig({ nom, prenom, dn, data: null, loading: false });
        return;
      }
      const d = await r.json();
      setDrillDirig({ nom, prenom, dn, data: d, loading: false });
    } catch {
      setDrillDirig({ nom, prenom, dn, data: null, loading: false });
    }
  };
  ...
  return (
    <>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                {data?.dirigeants.map((d, i) => {
                  ...
                  return (
                    ...
                    onClick={() => {
                      if (d.nom && d.prenom) {
                        loadDrillDirig(String(d.nom), String(d.prenom), dateN);
                      }
                    }}>
                      {/* Avatar */}
                      <div style={{
      ...
      {/* Modal drill-down dirigeant : toutes les infos personnelles + OSINT */}
      {drillDirig && (
        <div
          role="dialog"
          style={{
          ...
          onClick={() => setDrillDirig(null)}
        >
          <div
            style={{
              width: "min(880px, 95vw)", maxHeight: "92vh",
            ...
            <div style={{ padding: "14px 18px", borderBottom: "1px solid var(--border-subtle)", display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 16, fontWeight: 700 }}>
                  {drillDirig.prenom} {drillDirig.nom}
                </div>
                <div style={{ fontSize: 11, color: "var(--text-tertiary)", marginTop: 2 }}>
                  Profil dirigeant complet · INPI + SCI patrimoine + OSINT + sanctions
                  {drillDirig.dn ? ` · né(e) ${drillDirig.dn}` : ""}
                </div>
              </div>
              <button className="dem-btn" onClick={() => setDrillDirig(null)}>Fermer</button>
            </div>

            <div style={{ flex: 1, overflowY: "auto", padding: 18 }}>
              {drillDirig.loading && <div style={{ color: "var(--text-tertiary)" }}>Chargement…</div>}
              {!drillDirig.loading && !drillDirig.data && (
                <div style={{ color: "var(--text-tertiary)" }}>Aucune donnée pour ce dirigeant.</div>
              )}
              {!drillDirig.loading && drillDirig.data && (
                <DirigDrillContent data={drillDirig.data} />
              )}
            </div>
          </div>
        </div>
...
function DirigDrillContent({ data }: { data: Record<string, unknown> }) {
  ...
  return (
    <div>
      ...
      {dvf && Number(dvf.n_zones) > 0 && (
        <Section title="Transactions immobilières DVF (zones SCI)">
          {arr(dvf.by_cp).length === 0 && Array.isArray(dvf.by_cp) && (dvf.by_cp as unknown[]).map((row, k) => {
            const r = row as Record<string, unknown>;
            return <Row key={k} label={`CP ${String(r.code_postal || "")}`} value={`${r.n} transactions · total ${fmt(r.total)}`} />;
          })}
          {Array.isArray(dvf.by_cp) && (dvf.by_cp as unknown[]).map((row, k) => {
            const r = row as Record<string, unknown>;
            return <Row key={k} label={`CP ${String(r.code_postal || "")}`} value={`${r.n} transactions · total ${fmt(r.total)}`} />;
          })}
        </Section>
